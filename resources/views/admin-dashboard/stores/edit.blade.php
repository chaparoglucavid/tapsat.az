...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">{{ t_db('general', 'edit_store') }}</h5>
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('stores.update', $store->uuid) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="phone_number" class="form-control" required value="{{ old('phone_number', $store->phone_number) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ t_db('general', 'email') }}</label>
                        <input type="email" name="email" class="form-control" required value="{{ old('email', $store->email) }}">
                    </div>
                </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="d-flex flex-wrap gap-3">
                            @php 
                                ...
                                $selectedDays = $store->working_days ?? [];
                                if(!is_array($selectedDays)) $selectedDays = explode(',', $selectedDays);
                            @endphp
                            @foreach($days as $day)
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="working_hours" class="form-control" placeholder="e.g. 09:00 - 18:00" required value="{{ old('working_hours', $store->working_hours) }}">
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        @if($store->logo)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $store->logo) }}" width="100" class="rounded">
                            </div>
                        @endif
                    ...
                    <div class="col-md-6">
                        ...
                        @if($store->banner_image)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $store->banner_image) }}" width="100" class="rounded">
                            </div>
                        @endif
