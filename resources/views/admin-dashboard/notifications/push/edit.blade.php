...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">{{ t_db('general', 'edit_notification') }}</h5>
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('push-notifications.update', $notification->uuid) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <textarea name="message" class="form-control" rows="3" required>{{ old('message', $notification->message) }}</textarea>
                </div>

                <div class="mb-3">
                    <label class="form-label">{{ t_db('general', 'deep_link') }} ({{ t_db('general', 'optional') }})</label>
                    <input type="url" name="deep_link" class="form-control" value="{{ old('deep_link', $notification->deep_link) }}" placeholder="https://example.com/page">
                </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-check custom-option custom-option-icon">
                                <label class="form-check-label custom-option-content" for="target_all">
                                    ...
                                    <input name="target_type" class="form-check-input" type="radio" value="all" id="target_all" {{ $notification->target_type == 'all' ? 'checked' : '' }} />
                                </label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check custom-option custom-option-icon">
                                <label class="form-check-label custom-option-content" for="target_users">
                                    ...
                                    <input name="target_type" class="form-check-input" type="radio" value="users" id="target_users" {{ $notification->target_type == 'users' ? 'checked' : '' }} />
                                </label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check custom-option custom-option-icon">
                                <label class="form-check-label custom-option-content" for="target_category">
                                    ...
                                    <input name="target_type" class="form-check-input" type="radio" value="category" id="target_category" {{ $notification->target_type == 'category' ? 'checked' : '' }} />
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="target_users_container" class="d-none animate__animated animate__fadeIn">
                        ...
                        <select name="target_users[]" class="form-select select2" multiple>
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ is_array($notification->target_value) && in_array($user->id, $notification->target_value) ? 'selected' : '' }}>
                    ...
                    <div id="target_category_container" class="d-none animate__animated animate__fadeIn">
                        ...
                        <select name="target_category" class="form-select select2">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ $notification->target_type == 'category' && $notification->target_value == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary">
                        <i class="bx bx-save"></i> {{ t_db('general', 'update_draft') }}
                    </button>
                </div>
            </form>
        </div>
